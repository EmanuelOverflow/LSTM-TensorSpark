...
class RNN():
    ...
    def map_data_by_key(self):
        weight_forget = self.s.run(self.layer.weight_forget)
        weight_input = self.s.run(self.layer.weight_input)
        weight_output = self.s.run(self.layer.weight_output)
        weight_C = self.s.run(self.layer.weight_C)
        biases_forget = self.s.run(self.layer.biases_forget)
        biases_input = self.s.run(self.layer.biases_input)
        biases_C = self.s.run(self.layer.biases_C)
        biases_output = self.s.run(self.layer.biases_output)
        return [
            ("wf", weight_forget),
            ("wi", weight_input),
            ("wo", weight_output),
    ...
    def _epoch_fit(self, e):
        e_time = time.time()
        print 'Epoch: %d' % (e + 1)
        print

        map(self._input_loop, xrange(len(self.data)), self.data, self.labels)
    ...
    def _input_loop(self, i, in_data, label):
        # print "Partition: %d - Input: %d/%d\r" % (partition, (i + 1), len(data))
        #s.run(self.layer.fit_next(in_data, s))#, feed_dict={input_data: data})
        self.layer.fit_next(in_data, self.s)
        if i%self.mini_batch == 0:
            minimizer = self.layer.minimize(in_data, label, self.s, self.optimizer)
            if not self.var_initialized:
                tf.initialize_all_variables().run()
                self.var_initialized = True
            self.s.run(minimizer)
        return []
            #s.run(self.layer.minimize(in_data, label, s, self.optimizer))

    def set_minibatch(self, minibatch):
        self.mini_batch = minibatch

    def set_session(self, session):
        self.s = session
        self.var_initialized = False

    def set_dataset(self, datas, mini_batch=None, normalize=True):
        self.data = []
        self.labels = []
        if len(datas) <= 1:
            return datas

        data_append = self.data.append
        labels_append = self.labels.append

        for d in datas: # si puo spostare in una map
            if len(d) <= 1:
                continue
        ...
        if normalize:
            self.data = min_max_normalizer(self.data)

        if not mini_batch:
            self.mini_batch = len(self.data) - 1
        else:
    ...
    def fit_layer_fast(self):
        ...
        map(self._epoch_fit, xrange(self.epoch))

        batch_weights = self.map_data_by_key()
...
def train_rnn(partition, multilayer_props, target, epoch=100, minibatch=None):
    ...
    with tf.Session() as s:
        rnn.set_dataset(partition[0][1])
        rnn.set_session(s)
        rnn.set_minibatch(minibatch)
        ret = rnn.fit_layer_fast()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Fast RNN-LSTM built on Tensorflow')
